...
class Magento(MagentoConnection):
    #PRODUCT OPERATIONS
    def getProductInfo(self, sku_or_id, type='sku'):
        """Gives the product info"""
        return self._call('catalog_product.info', [sku_or_id, None, None, type])
    ...
    def listProducts(self, filters=None, store_view=None):
        if filters:
            if store_view:
